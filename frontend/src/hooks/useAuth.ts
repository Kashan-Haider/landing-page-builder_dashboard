...
export const useAuth = () => {
  ...
  const clearAuthData = useCallback(() => {
    ...
    // Also remove legacy keys for backward compatibility
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      const userStr = localStorage.getItem('authUser') || localStorage.getItem('adminUser');
  ...
  // Simple redirect function - always redirect to employee login
  const redirectToRoleBasedLogin = () => {
